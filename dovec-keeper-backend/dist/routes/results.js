...
async function getSubcategoriesForSurvey(survey) {
    try {
        ...
        for (const categoryNameOrId of categoryNamesOrIds) {
            ...
            try {
                ...
                category = await Category_1.default.findOne({ name: categoryNameOrId });
            }
            if (category && category._id) {
                // Fetch subcategories for this category and populate category name
                const subcategories = await Subcategory_1.default.find({ category: category._id }).populate("category", "name");
                // Filter out any null subcategories and add category name to each
                const validSubcategories = (subcategories || []).filter((sub) => sub && sub._id).map((sub) => {
...
function calculateScores(answers, questions, userKpi = 0) {
    const scores = {
    ...
    answers.forEach((a) => {
        ...
        try {
            if (mongoose_1.default.Types.ObjectId.isValid(qId)) {
                const objId = new mongoose_1.default.Types.ObjectId(qId);
    ...
    questions.forEach((question) => {
        ...
        const isTeam = categoryName.includes("team effect") ||
        ...
        const isExecutive = categoryName.includes("executive observation") ||
...
router.get("/", auth_1.protect, async (req, res) => {
    try {
        ...
        let responsesQuery = { status: "submitted" };
        ...
        const allResponses = await Response_1.default.find(responsesQuery)
        ...
        let responses = allResponses.filter((response) => {
        ...
        const employeeKpiCache = new Map();
        for (const response of responses) {
            ...
            if (userKpi === undefined) {
                const userDoc = await User_1.default.findById(employee._id).select("kpi");
            ...
            if (questionsForCalculation.length === 0) {
                ...
                questionsForCalculation = (subcategories || []).filter((subcat) => subcat && subcat._id).map((subcat) => ({
                ...
                const subcategoryQuestions = (subcategories || []).filter((subcat) => subcat && subcat._id).map((subcat) => ({
                    id: subcat._id.toString(),
                    _id: subcat._id,
                    text: subcat.name,
                    name: subcat.name,
...
router.get("/:employeeId", auth_1.protect, async (req, res) => {
    try {
        ...
        const allResponses = await Response_1.default.find({ status: "submitted" })
            .populate("employee", "name email role department kpi")
            .populate("survey", "title questions categories")
            .sort({ submittedAt: -1 });
        ...
        if (req.user.role === "manager") {
            ...
            if (!isViewingSelf) {
                const targetEmployee = allResponses.find((r) => {
                ...
                if (targetEmployee) {
                    const emp = targetEmployee.employee;
        ...
        const employee = responses[0].employee;
